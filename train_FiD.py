...
import numpy as np
from transformers import T5Tokenizer, T5ForConditionalGeneration, T5Config, get_constant_schedule_with_warmup
import argparse
...
def train():
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if args.eval_epoch!=0 and epoch%args.eval_epoch==0:
            # train
            train_scores_,_ = evaluation(args, model, tokenizer, train_dataloader)
            ...
            if args.local_rank in [-1,0]:
                ...
                if args.save_model_every_epoch:
                    ...
                    torch.save(optimizer.state_dict(), os.path.join(output_dir, "optimizer_%d.pt"%epoch))
                    torch.save(scheduler.state_dict(), os.path.join(output_dir, "scheduler_%d.pt"%epoch))
...
def load_datasets(args, tokenizer):
    ...
    train_dataset = FiDDataset(train_data, tokenizer, args.max_length, args.answer_max_length, args.n_docs)
    if args.distributed:
        # OK - legacy
        val_data = load_data(args.val_data, args.local_rank, args.distributed)
    else:
        val_data = load_jsonl(args.val_data)
    val_dataset = FiDDataset(val_data, tokenizer, args.max_length, args.answer_max_length, args.n_docs)
    # pair 추가
